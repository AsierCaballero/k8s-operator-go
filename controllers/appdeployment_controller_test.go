...
func TestAppDeploymentEnvVars(t *testing.T) {
	...
	g.Expect(deploy.Spec.Template.Spec.Containers[0].Env).To(BeEmpty())
...
func TestAppDeploymentDeploymentLabels(t *testing.T) {
	...
	g.Expect(deploy.Spec.Selector.MatchLabels).To(HaveKeyWithValue("app.kubernetes.io/instance", app.Name))
}
